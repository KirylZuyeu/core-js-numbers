...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  return Math.acos(vectorA + vectorB);
}
...
function roundToPowerOfTen(num, pow) {
  ...
  if (pow === 0) {
    ...
    const decimal = 10 ** pow;
    resultNum = Math.round(num / decimal) * decimal;
  }
  return resultNum;
}
...
function getSumOfDigits(num) {
  ...
  const sum = arrNums.reduce((a, b) => +a + +b);
  return sum;
}
...
function getSumOfNumbers(...args) {
  return args.reduce((acc, e) => acc + parseFloat(e), 0).toFixed(1);
}
...
function getCountOfOddNumbers(number) {
  ...
  if (number > 0) {
    ...
    result = Math.abs(Math.floor(number / 2));
  }
  return result;
}
